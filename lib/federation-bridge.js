  ...
  return async (req, res, next) => {
    try {
      ...
      if (notFound || notAcceptable) {
        // Well-known discovery endpoints (WebFinger, NodeInfo, host-meta) are
        // unauthenticated. Return Fedify's 404/406 directly instead of falling
        // through to next() — otherwise downstream auth middleware 302-redirects
        // the request to a login page. RFC 7033 §4.2 requires 404 for an unknown
        // WebFinger resource; a 302 makes remote servers misread it as an error.
        if ((req.originalUrl || req.path || "").startsWith("/.well-known/")) {
          return await sendFedifyResponse(res, response);
        }
        return next();
      }
