...
export function jf2ToActivityStreams(properties, actorUrl, publicationUrl, options = {}) {
  ...
  if (postType === "like") {
    ...
    // Stable, dereferenceable id (the like post's own permalink) so outbox
    // entries are individually addressable.
    if (properties.url) like.id = resolvePostUrl(properties.url, publicationUrl);
    return like;
  }
  ...
  if (postType === "repost") {
    ...
    if (properties.url) announce.id = resolvePostUrl(properties.url, publicationUrl);
    return announce;
  }
  ...
  // Safe-by-default: ONLY "public" is addressed to the public collection.
  // "unlisted" is public-in-cc; everything else (followers, private, direct, or
  // any unrecognized value) is followers-only so an unmapped visibility can
  // never silently federate a private post publicly.
  ...
  const object = {
    ...
    url: postUrl,
    to: isPublic ? [PUBLIC] : [followersUrl],
    cc: isPublic ? [followersUrl] : isUnlisted ? [PUBLIC] : [],
  };
  ...
  return {
    "@context": "https://www.w3.org/ns/activitystreams",
    // Stable Create-wrapper id distinct from the inner object id (postUrl).
    ...(postUrl ? { id: `${postUrl}#create` } : {}),
    type: "Create",
    actor: actorUrl,
    object,
  };
...
export function jf2ToAS2Activity(properties, actorUrl, publicationUrl, options = {}) {
  ...
  if (postType === "like") {
    ...
    return new Like({
      // Stable id so retried deliveries dedupe (Mastodon double-counts id-less Likes)
      id: new URL(`${actorUri.href}#likes/${crypto.randomUUID()}`),
      actor: actorUri,
  ...
  if (postType === "repost") {
    ...
    return new Announce({
      // Stable id so retried deliveries dedupe (id-less Announces re-boost on Mastodon)
      id: new URL(`${actorUri.href}#announces/${crypto.randomUUID()}`),
      actor: actorUri,
      object: new URL(repostOf),
      to: new URL("https://www.w3.org/ns/activitystreams#Public"),
      // cc followers so servers that key notification propagation on cc
      // (Misskey/Akkoma/some Lemmy) surface the boost to our followers there.
      cc: new URL(`${actorUri.href.replace(/\/$/, "")}/followers`),
    });
  }
  ...
  const noteOptions = {
    // Fedify's Object constructor property is `attribution` (it maps to
    // `attributedTo` in JSON-LD). Passing `attributedTo` here is silently
    // dropped by Fedify, shipping author-less Notes/Articles — remote servers
    // then can't attribute a standalone-fetched Note (Mastodon only works
    // because it falls back to the Create's actor).
  ...
  // Addressing based on visibility. Safe-by-default: PUBLIC is used ONLY for an
  // explicit "public". "unlisted" is public-in-cc; ANY other value (followers,
  // private, direct, unknown) is followers-only — a private/DM post must never
  // fall through to public addressing.
  // - "public":    to: PUBLIC, cc: followers (+ reply author)
  // - "unlisted":  to: followers, cc: PUBLIC (+ reply author)
  // - else:        to: followers (+ reply author), no PUBLIC
  ...
  return new Create({
    // Deterministic id derived from the post (the Note already has a stable id),
    // so servers that dedupe on the Create's id don't double-process retries.
    id: postUrl
      ? new URL(`${postUrl}#create`)
      : new URL(`${actorUri.href}#creates/${crypto.randomUUID()}`),
    actor: actorUri,
    object,
    // Mirror the object's addressing + published onto the Create — AS2 activities
    // do not inherit their object's to/cc, and some servers classify visibility
    // from the Create itself, not the inner object.
    ...(noteOptions.published ? { published: noteOptions.published } : {}),
    ...(noteOptions.to ? { to: noteOptions.to } : {}),
    ...(noteOptions.tos ? { tos: noteOptions.tos } : {}),
    ...(noteOptions.cc ? { cc: noteOptions.cc } : {}),
    ...(noteOptions.ccs ? { ccs: noteOptions.ccs } : {}),
  });
}
