...
import { Article, Application, Emoji, Hashtag, Mention, Question, Service } from "@fedify/vocab";
...
export async function extractObjectData(object, options = {}) {
  ...
  // In-reply-to — Fedify uses replyTargetId (non-fetching). Store null (not "")
  // for non-replies so the timeline reply-exclusion filter ({$in:[null,""]})
  // works; the empty string used to be silently excluded by {$exists:false}.
