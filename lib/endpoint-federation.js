...
export async function unfollowActor(self, actorUrl) {
  ...
  try {
    ...
    if (needsDirectFollow(actorUrl)) {
      ...
      // Reuse the original Follow's stored id so GoToSocial/Akkoma can match the
      // Undo to the follow record and actually remove us (id-less Undos are dropped).
      const stored = await self._collections.ap_following.findOne({ actorUrl });
      const followOpts = {
        actor: ctx.getActorUri(handle),
        object: new URL(actorUrl),
      };
      if (stored?.followActivityId) {
        followOpts.id = new URL(stored.followActivityId);
      }
      const follow = new Follow(followOpts);
      const undo = new Undo({
