...
export async function followActor(self, actorUrl, actorInfo = {}) {
  ...
  try {
    const { Follow } = await import("@fedify/vocab");
    ...
    if (needsDirectFollow(actorUrl)) {
      ...
      // Stable Follow id, stored below so the matching Undo can reference it —
      // GoToSocial/Akkoma need the original Follow's id to locate + remove it.
      followActivityId = `${ctx.getActorUri(handle).href}#follows/${crypto.randomUUID()}`;
...
export async function unfollowActor(self, actorUrl) {
  ...
  try {
    const { Follow, Undo } = await import("@fedify/vocab");
    const handle = self.options.actor.handle;
    const ctx = self._federation.createContext(
      new URL(self._publicationUrl),
      { handle, publicationUrl: self._publicationUrl },
    ...
    if (needsDirectFollow(actorUrl)) {
      ...
      // Reuse the original Follow's stored id (v3.13.15+) so servers that match
      // Undo(Follow) by the inner Follow id can reconcile it. Legacy follows have
      // none stored; fall back to a synthetic id so the inner Follow is always
      // well-formed. Reconciliation also works on the (actor, object) tuple —
      // the universal match — which is always present, so id-less legacy follows
      // still unfollow on tuple-matching servers.
      const stored = await self._collections.ap_following.findOne({ actorUrl });
      const follow = new Follow({
        id: stored?.followActivityId
          ? new URL(stored.followActivityId)
          : new URL(`${ctx.getActorUri(handle).href}#follows/${crypto.randomUUID()}`),
        actor: ctx.getActorUri(handle),
        object: new URL(actorUrl),
      });
      const undo = new Undo({
        id: new URL(`${ctx.getActorUri(handle).href}#undos/${crypto.randomUUID()}`),
        actor: ctx.getActorUri(handle),
        object: follow,
      });
...
export async function broadcastActorUpdate(self) {
  ...
  try {
    const { Update } = await import("@fedify/vocab");
    const handle = self.options.actor.handle;
    const ctx = self._federation.createContext(
      new URL(self._publicationUrl),
      { handle, publicationUrl: self._publicationUrl },
    ...
    const update = new Update({
      id: new URL(`${ctx.getActorUri(handle).href}#updates/${crypto.randomUUID()}`),
      actor: ctx.getActorUri(handle),
      object: actor,
...
export async function broadcastDelete(self, postUrl) {
  ...
  try {
    const { Delete } = await import("@fedify/vocab");
    const handle = self.options.actor.handle;
    const ctx = self._federation.createContext(
      new URL(self._publicationUrl),
      { handle, publicationUrl: self._publicationUrl },
    ...
    const del = new Delete({
      id: new URL(`${ctx.getActorUri(handle).href}#deletes/${crypto.randomUUID()}`),
      actor: ctx.getActorUri(handle),
