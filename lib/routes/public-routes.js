...
import { resolvePostUrl } from "../core/posts.js";
...
export function buildContentNegotiationRoutes(self) {
  ...
  router.get("{*path}", async (req, res, next) => {
    ...
    try {
      ...
      // core tolerates a trailing slash on both the post and tombstone lookups.
      const requestUrl = `${self._publicationUrl}${req.path.slice(1)}`;
      const { post, tombstone } = await resolvePostUrl(
        { posts, ap_tombstones: self._collections?.ap_tombstones },
        requestUrl,
      );
