...
export function setupFederation(options) {
  ...
  const federation = createFederation({
    kv,
    queue,
    // Pin the canonical origin so actor IDs / inbox URLs are deterministic
    // regardless of the parent Express app's `trust proxy` setting. Behind nginx
    // the request-derived origin is already correct; pinning guards against a
    // misconfigured proxy serialising an internal-host URI remotes can't deref.
    ...(publicationUrl ? { origin: new URL(publicationUrl).origin } : {}),
  });
  ...
  federation
    ...
    .setKeyPairsDispatcher(async (ctx, identifier) => {
      ...
      // --- RSA key pair (HTTP Signatures) ---
      // Legacy installs store RSA as PEM; newer installs (and freshly generated
      // ones) store JWK. Load whichever exists. If NEITHER exists — a net-new
      // deployment with no migrated keys — generate + persist RSA so the actor
      // can sign HTTP requests. Without an RSA key the actor is served with no
      // CryptographicKey and every server requiring HTTP Signatures (most of
      // Mastodon) rejects our deliveries. RSA is loaded first so keyPairs[0] is RSA.
      ...
      } else if (rsaDoc?.publicKeyJwk && rsaDoc?.privateKeyJwk) {
        try {
          const publicKey = await importJwk(rsaDoc.publicKeyJwk, "public");
          const privateKey = await importJwk(rsaDoc.privateKeyJwk, "private");
          keyPairs.push({ publicKey, privateKey });
          rsaLoaded = true;
        } catch (error) {
          console.warn(
            "[ActivityPub] Could not import RSA JWK keys:",
            error.message,
          );
        }
      }
      ...
      if (!rsaLoaded) {
        try {
          ...
          await collections.ap_keys.insertOne({
            type: "rsa",
            publicKeyJwk: await exportJwk(rsa.publicKey),
            privateKeyJwk: await exportJwk(rsa.privateKey),
            createdAt: new Date().toISOString(),
          });
          keyPairs.push(rsa);
          console.info("[ActivityPub] Generated and persisted RSA key pair");
        } catch (error) {
          console.warn(
            "[ActivityPub] Could not generate RSA key pair:",
            error.message,
          );
        }
      }
  ...
  federation.setWebFingerLinksDispatcher((_ctx, _resource) => {
    const interactionBase = `${publicationUrl.replace(/\/$/, "")}${mountPath}/authorize_interaction`;
    return [
  ...
  // Deduplicate at-least-once deliveries by (inbox, activity id). AP delivery is
  // at-least-once — remote servers retry on timeout/5xx and the shared inbox can
  // receive the same signed payload more than once. Without this, a duplicate
  // Create/Like enqueues twice (duplicate notifications/timeline rows) and a
  // duplicate Follow sends two Accepts. Uses Fedify's own KV (activityIdempotence
  // key prefix) — NOT a (kv, ttl) argument (that API does not exist in 2.x).
  inboxChain.withIdempotency("per-inbox");

  // Surface inbox handler exceptions. Without an onError hook, Fedify answers the
  // remote 202 Accepted and logs the error only through its own LogTape sink, so
  // Mongo write failures / unexpected payload shapes are invisible in app logs.
  inboxChain.onError((_ctx, error) => {
    const msg = error?.message || String(error);
    // Transient remote-server flakiness — an empty/truncated body during Fedify's
    // internal key/actor/object resolution ("Unexpected end of JSON input"), a
    // network blip, or a 5xx — is not our bug and self-heals on redelivery. Log
    // it at warn so it doesn't read as an application error. Everything else gets
    // the full stack for diagnosis. Either way the POST still 202s (non-fatal).
    const transient =
      /Unexpected end of JSON input|Unexpected token|fetch failed|network|ECONNRESET|ETIMEDOUT|timed? out|50[234]/i.test(
        msg,
      );
    if (transient) {
      console.warn("[ActivityPub] Inbox transient error (remote):", msg);
    } else {
      console.error(
        "[ActivityPub] Inbox handler error:",
        error?.stack || msg,
      );
    }
  });
  ...
  federation.setNodeInfoDispatcher("/nodeinfo/2.1", async () => {
    // localPosts is conventionally published, non-interaction posts — exclude
    // deleted posts and like/repost interactions so the crawler-visible count
    // isn't inflated.
    const postsCount = collections.posts
      ? await collections.posts.countDocuments({
          "properties.deleted": { $exists: false },
          "properties.post-type": { $nin: ["like", "repost"] },
        })
      : 0;
    ...
    return {
      ...
      metadata: {
        ...
        staffAccounts: [
          `${publicationUrl.replace(/\/$/, "")}${mountPath}/users/${handle}`,
        ],
      },
    };
  });
  ...
  if (debugDashboard) {
    // The dashboard exposes traces, queued delivery payloads, signature details,
    // and correlated logs. Refuse to start it unauthenticated — a password-less
    // dashboard on a public host leaks all of that to anyone who reaches the URL.
    if (!debugPassword) {
      throw new Error(
        "[ActivityPub] debugDashboard requires debugPassword — refusing to start an unauthenticated debug dashboard",
      );
    }
    const debugOptions = {
      path: `${mountPath}/__debug__`,
      auth: { type: "password", password: debugPassword },
    };
    activeFederation = createFederationDebugger(federation, debugOptions);
    console.info(
      `[ActivityPub] Debug dashboard enabled at ${mountPath}/__debug__/ (password-protected)`,
    );
  }
  ...
  federation
    .setFollowersDispatcher(
      ...
      async (ctx, identifier, cursor, filter) => {
        ...
        // FEP-8fcf partial-followers collection: when Fedify serves the
        // Collection-Synchronization `url` to a remote instance, it passes that
        // instance's base URL as `filter`. Return only followers whose id shares
        // that scheme+authority so we don't leak (or slowly serialise) the entire
        // followers list to every server we deliver to.
        const serverFilter = filter
          ? {
              actorUrl: {
                $regex: `^${filter.origin.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}/`,
              },
            }
          : {};
        const fkey = filter ? `:${filter.host}` : "";
        ...
        if (cursor == null) {
          const docs = await cachedQuery(`col:followers:recipients${fkey}`, COLLECTION_CACHE_TTL, async () => {
        ...
        const [docs, total] = await cachedQuery(`col:followers:page:${cursor}${fkey}`, COLLECTION_CACHE_TTL, async () => {
          const d = await collections.ap_followers
            ...
            .limit(pageSize)
            .toArray();
          const t = await collections.ap_followers.countDocuments(serverFilter);
          return [d, t];
        });
...
function setupFeatured(federation, mountPath, handle, collections, publicationUrl) {
  federation.setFeaturedDispatcher(
    ...
    // Without a counter the featured collection ships no `totalItems`, which
    // some clients require to render the pinned-posts count.
    .setCounter(async (ctx, identifier) => {
      if (identifier !== handle) return 0;
      if (!collections.ap_featured) return 0;
      return await collections.ap_featured.countDocuments();
    });
}
...
export async function buildPersonActor(
  ...
  if (profile.summary) {
    // AS2 treats `summary` as HTML. Stored profile summaries are plain text with
    // newlines, so Mastodon renders them as one unbroken block. Convert to
    // <p>/<br> (escaping any literal angle brackets). If the value already
    // contains markup, pass it through untouched.
    const summary = profile.summary;
    if (/<[a-z][\s\S]*>/i.test(summary)) {
      personOptions.summary = summary;
    } else {
      const esc = (t) =>
