 ...
 * never touched. Runs once (ap_kv marker): from beta.6 ingest creates the rows,
 * so re-scanning the notification history on every restart would be pure cost.
 *
 * @param {object} collections
 * @returns {Promise<number>} rows created
 */
export async function backfillNotificationContext(collections) {
  const { ap_kv } = collections;
  const MARKER = "migration:notification-context";
  if (await ap_kv?.findOne({ _id: MARKER })) return 0;

  const created = await insertNotificationContextRows(collections);
  await ap_kv?.updateOne(
    { _id: MARKER },
    { $set: { value: { completed: true, created, date: new Date().toISOString() } } },
    { upsert: true },
  );
  return created;
}

async function insertNotificationContextRows(collections) {
