...
import { getEngagementCounts } from "./core/interactions.js";
...
export async function postProcessItems(items, options = {}) {
  ...
  // 6. Engagement counts — the same rule the Mastodon API serialises
  //    (core/interactions#getEngagementCounts). Needs ap_profile/ap_activities,
  //    so only when the caller passes collections; ap_timeline items only.
  if (options.collections) {
    const liked = new Set();
    const boosted = new Set();
    for (const [uid, state] of Object.entries(interactionMap)) {
      if (state.like) liked.add(uid);
      if (state.boost) boosted.add(uid);
    }
    const counts = await getEngagementCounts(options.collections, items, {
      favouritedIds: liked,
      rebloggedIds: boosted,
    });
