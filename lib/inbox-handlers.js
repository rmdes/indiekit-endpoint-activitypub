...
import { getFollowedTags } from "./core/tags.js";
...
export async function handleCreate(item, collections, ctx, handle) {
  ...
  // Posts that notify us (a reply to one of our posts, or a mention) must also
  // exist as timeline rows, or nothing can show or act on them — see the end of
  // this handler. `storedInFeed` records whether a feed path already did it.
  let notifiesUs = false;
  let storedInFeed = false;
  ...
  // Materialize inline tags once. Fedify vocab objects expose getTags() (async) —
  // NOT a .tag property (object.tag is always undefined), and object.tagIds is empty
  // for anonymous inline Mention/Hashtag. getTags() is the only reliable path.
  const tagList = [];
  try {
    ...
    console.warn("[inbox-handlers] Failed to read tags:", error.message);
  }
  ...
  if (tagList.length) {
    for (const tag of tagList) {
      if (tag instanceof Mention && tag.href?.href === ourActorUrl) {
        ...
        await addNotification(collections, {
          uid: object.id?.href || `mention:${actorUrl}:${crypto.randomUUID()}`,
          url: object.url?.href || object.id?.href || "",
          type: "mention",
          actorUrl: actorInfo.url,
          actorName: actorInfo.name,
  ...
  if (following) {
    ...
    try {
      const postHashtags = tagList
        .filter((t) => t instanceof Hashtag && t.name)
        .map((t) => t.name.toString().replace(/^#/, "").toLowerCase());

      if (postHashtags.length > 0) {
        const followedTags = await getFollowedTags(collections);
        // Any follow state counts: a tags.pub follow delivers tagged posts too.
        const followedSet = new Set(followedTags.map((t) => t.tag));
  ...
  // A reply or mention from an account we don't follow: keep it as thread
  // context. isContext keeps it out of every feed, but it becomes a real status
  // — the reader's post detail and Mastodon /context list it under our post,
  // and the Mastodon notification resolves to its id instead of a
  // notification-id stand-in that no status route can find (beta.6).
  if (notifiesUs && !storedInFeed) {
    try {
      const timelineItem = await extractObjectData(object, {
        actorFallback: actorObj,
        documentLoader: authLoader,
      ...
      console.error("[inbox-handlers] Failed to store reply/mention context:", error.message);
    }
  }
}
...
export async function handleDelete(item, collections) {
  ...
  // Ownership gate: Fedify verifies the Delete is signed by its actor, but NOT
  // that the actor authored the target object. Without this, any authenticated
  // fediverse actor could send Delete{object:<any uid>} and purge arbitrary
  // items (incl. our own) from the reader timeline. Only honour the Delete when
  // the signing actor shares the object's origin host, or authored the stored
  // item. If the item isn't in our timeline there's nothing to spoof-delete.
  const stored = await collections.ap_timeline.findOne({ uid: objectId });
  if (stored) {
    let sameOrigin = false;
    try {
      sameOrigin = new URL(item.actorUrl).hostname === new URL(objectId).hostname;
    } catch {
      sameOrigin = false;
    }
    if (!sameOrigin && stored.author?.url !== item.actorUrl) {
      console.warn(
        `[inbox-handlers] Rejected Delete of ${objectId} from non-owner ${item.actorUrl}`,
      );
      return;
    }
  }
...
export async function handleUpdate(item, collections, ctx, handle) {
  ...
  if (object && (object instanceof Note || object instanceof Article)) {
    ...
    if (objectUrl) {
      try {
        // Extract updated content — sanitize HTML before storing (same as the
        // Create path via extractObjectData). Without this a followed actor can
        // edit a post to inject <script>/<iframe> that lands unsanitized in
        // ap_timeline and executes in the reader.
        const rawHtml = object.content?.toString() || "";
        const contentHtml = sanitizeContent(rawHtml);
        const contentText = object.source?.content?.toString() || rawHtml.replace(/<[^>]*>/g, "");
...
export async function handleFlag(item, collections, ctx, handle) {
  try {
    ...
    // Resolve the reporter's avatar via extractActorInfo — `actorObj.iconUrl` is
    // not a valid Fedify accessor (always undefined); getIcon() is the real path.
    const actorInfo = actorObj
      ? await extractActorInfo(actorObj, { documentLoader: authLoader })
      : null;
