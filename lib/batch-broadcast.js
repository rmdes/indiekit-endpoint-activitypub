  ...
  for (let i = 0; i < uniqueRecipients.length; i += batchSize) {
    ...
    try {
      await ctx.sendActivity({ identifier: handle }, recipients, activity, {
        ...
        // Order deliveries per object so a rapid Update-then-Delete of the same
        // post can't race to the same inbox out of order.
        ...(objectUrl ? { orderingKey: objectUrl } : {}),
        // NB: syncCollection (FEP-8fcf) is NOT set here — it only applies to the
        // "followers" collection overload of sendActivity, where Fedify can
        // compute the follower digest. With an explicit recipients array it is
        // silently ignored, so setting it would be misleading dead config. The
        // syndicator's "followers" send carries it correctly.
      });
