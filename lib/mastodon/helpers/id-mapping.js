...
/**
 * Is this client-supplied account id the LOCAL account?
 *
 * Clients hold the id minted by accountId() — sha256(profile.url) — so that is
 * the canonical comparison. The profile's Mongo _id is accepted as a legacy
 * fallback (clients from before ids moved to the url hash). Comparing ONLY
 * _id was the bug that made /accounts/:id/followers etc. return [] for
 * everyone, including the local account.
 *
 * @param {string} id - Account id from the request
 * @param {object|null} profile - The ap_profile document
 * @returns {boolean}
 */
export function isLocalAccountId(id, profile) {
  if (!id || !profile) return false;
  if (profile.url && remoteActorId(profile.url) === id) return true;
  return profile._id?.toString() === id;
}
