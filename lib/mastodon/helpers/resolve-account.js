...
/**
 * Get an authenticated document loader for signed fetches. Authorized-Fetch
 * (Secure Mode) Mastodon servers 401 unsigned GETs — without this, their
 * actors never resolve and degraded URL-derived usernames never get repaired.
 * @returns {Promise<object|undefined>}
 */
async function getSignedLoader(ctx, handle) {
  try {
    return await ctx.getDocumentLoader({ identifier: handle });
  } catch {
    return undefined;
  }
}
...
export async function resolveRemoteAccount(acct, pluginOptions, baseUrl) {
  ...
  try {
    ...
    // Determine lookup URI. Check http(s) FIRST — a profile URL like
    // "https://mastodon.social/@user" contains "@", so the acct branch would
    // mangle it into "acct:https://mastodon.social/@user". URLs are looked up
    // directly; only bare "user@domain" handles become acct: URIs.
    let actorUri;
    ...
    // Signed-first lookup (falls back to unsigned) so Authorized-Fetch servers resolve
    const documentLoader = await getSignedLoader(ctx, handle);
    ...
    // Cache stats + identity so embedded account objects in statuses can be
    // enriched AND degraded URL-derived usernames (numeric ids from
    // Authorized-Fetch servers we couldn't resolve at ingest) can be repaired.
    cacheAccountStats(actorUrl, {
      ...
      acct: username && domain ? `${username}@${domain}` : undefined,
...
export async function fetchRemoteCollectionMemberUrls(actorUrl, kind, pluginOptions, limit = 40) {
  ...
  try {
    ...
    // Signed-first so Authorized-Fetch servers serve the actor + collection pages
    const documentLoader = await getSignedLoader(ctx, handle);
    const loaderOptions = documentLoader ? { documentLoader } : {};
    const actor = await lookupWithSecurity(ctx, actorUrl, loaderOptions);
    if (!actor) return [];

    const collection = kind === "following"
      ? await actor.getFollowing(loaderOptions)
      : await actor.getFollowers(loaderOptions);
    ...
    if (urls.length === 0 && typeof collection.getFirst === "function") {
      const firstPage = await collection.getFirst(loaderOptions);
