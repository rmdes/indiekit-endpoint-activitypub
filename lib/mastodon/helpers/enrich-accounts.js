...
import { repairAuthorMetadata } from "../../core/timeline.js";
    ...
    for (let i = 0; i < unique.length; i += CONCURRENCY) {
      ...
      await Promise.allSettled(
        batch.map(async (url) => {
          ...
          // Durable repair: fix stored authors whose name is the numeric
          // placeholder. The WRITE lives in core — adapters do not write to
          // collections (boundary rule 5.2).
          if (account && !isDegradedUsername(account.username)) {
            const avatar =
              account.avatar && !account.avatar.endsWith("/default-avatar.svg")
                ? account.avatar
                : "";

            await repairAuthorMetadata(collections, url, {
