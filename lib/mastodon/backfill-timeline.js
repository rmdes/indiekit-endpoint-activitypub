...
export async function backfillTimeline(collections) {
  ...
  for (const post of allPosts) {
    ...
    const published = props.published || props.date || new Date().toISOString();
    ...
    const timelineItem = {
      ...
      published,
      createdAt: published,
      // Stage 2 fields. This runs after the startup migrations, so nothing else
      // will stamp them. An own post "arrived" when it was published (the
      // syndicator path stamps syndication time, which is the same instant).
