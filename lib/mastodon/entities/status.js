 ...
 */
export function serializeStatus(item, { baseUrl, favouritedIds, rebloggedIds, bookmarkedIds, pinnedIds, replyIdMap, replyAccountIdMap, counts } = {}) {
  if (!item) return null;
  ...
  if (item.type === "boost" && item.boostedBy) {
    ...
    // The boost wrapper and the inner (original) post MUST have distinct ids —
    // clients key their status cache by id, and reusing item._id for both made
    // the boost and the original clobber each other's state. Inner keeps
    // item._id (so favourite/reblog resolve to the right timeline doc); the
    // wrapper gets a deterministic distinct id.
    const boostWrapperId = remoteActorId(`boost:${uid}:${item.boostedAt || id}`);
  ...
  // Counts
  // Favourites/reblogs follow core/interactions#getEngagementCounts when the
  // route supplied them (via loadInteractionState); the ingest snapshot is the
  // fallback. Null (unknown) serialises as 0, as Mastodon clients expect.
  const engagement = counts?.get(uid);
  const repliesCount = item.counts?.replies ?? 0;
  const reblogsCount = engagement?.boosts ?? item.counts?.boosts ?? 0;
  const favouritesCount = engagement?.likes ?? item.counts?.likes ?? 0;
