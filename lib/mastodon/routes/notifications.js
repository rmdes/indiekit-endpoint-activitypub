...
router.get("/api/v1/notifications", tokenRequired, scopeRequired("read", "read:notifications"), async (req, res, next) => {
  try {
    ...
    // Build base filter — exclude notifications dismissed via the Mastodon API
    const baseFilter = { dismissed: { $ne: true } };
...
router.post("/api/v1/notifications/clear", tokenRequired, scopeRequired("write", "write:notifications"), async (req, res, next) => {
  try {
    const collections = req.app.locals.mastodonCollections;
    // Soft-dismiss, NOT delete: ap_notifications is shared with the plugin's
    // admin reader (and its read/unread history). A hard delete here would
    // destroy the owner's native notification history on one client tap.
    await collections.ap_notifications.updateMany({}, { $set: { dismissed: true } });
    res.json({});
  } catch (error) {
    next(error);
  }
...
router.post("/api/v1/notifications/:id/dismiss", tokenRequired, scopeRequired("write", "write:notifications"), async (req, res, next) => {
  try {
    ...
    // Soft-dismiss (shared store — see /clear). Never hard-delete here.
    await collections.ap_notifications.updateOne({ _id: objectId }, { $set: { dismissed: true } });
    res.json({});
  } catch (error) {
    next(error);
  }
