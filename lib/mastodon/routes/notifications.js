...
router.get("/api/v1/notifications", tokenRequired, scopeRequired("read", "read:notifications"), async (req, res, next) => {
  try {
    ...
    const interactionState = await loadInteractionState(collections, [...new Set(statusMap.values())]);
...
router.get("/api/v1/notifications/:id", tokenRequired, scopeRequired("read", "read:notifications"), async (req, res, next) => {
  try {
    ...
    const notification = serializeNotification(notif, {
      baseUrl,
      statusMap,
      interactionState: await loadInteractionState(collections, [...new Set(statusMap.values())]),
    });
...
async function batchFetchStatuses(collections, notifications) {
  ...
  // Replies/mentions serialize the TRIGGERING post (notif.url), likes/boosts
  // the target — fetch both, or a reply with a timeline row still falls back
  // to a notification-id stand-in no status route can resolve.
  const targetUrls = [
    ...new Set(
      notifications
        .flatMap((n) => [n.targetUrl, n.url, n.uid])
        .filter(Boolean),
    ),
  ];
  ...
  return getItemsByUrls(collections, targetUrls);
}
