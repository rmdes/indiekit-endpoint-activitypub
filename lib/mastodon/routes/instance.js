...
router.get("/api/v1/instance", async (req, res, next) => {
  try {
    ...
    // Approximate counts — display statistics, computed in core. The domain
    // count now aggregates in MongoDB rather than pulling every follower row
    // into memory to build a Set.
    let statusCount = 0;
    let domainCount = 0;
    try {
      ({ statusCount } = await getInstanceStats(collections));
      domainCount = await countFollowerDomains(collections);
    } catch {
      // Non-critical — statistics must never fail the instance endpoint.
    }
