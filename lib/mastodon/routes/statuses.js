...
router.get("/api/v1/statuses/:id/context", optionalToken, async (req, res, next) => {
  try {
    ...
    const isVisibleAnon = (it) => ["public", "unlisted"].includes(it.visibility);
    const visibleAncestors = req.mastodonToken ? ancestors : ancestors.filter(isVisibleAnon);
    const visibleDescendants = req.mastodonToken ? descendants : descendants.filter(isVisibleAnon);
