...
router.get("/api/v1/timelines/home", tokenRequired, scopeRequired("read", "read:statuses"), async (req, res, next) => {
  try {
    ...
    await enrichAccountStats(statuses, pluginOptions, baseUrl, req.app.locals.mastodonCollections);
...
router.get("/api/v1/timelines/public", async (req, res, next) => {
  try {
    ...
    const baseFilter = {
      isContext: { $ne: true },
      // Exclude replies. Non-replies may store inReplyTo as missing, null, OR
      // the empty string (legacy ingest wrote ""), so match all three — a bare
      // {$exists:false} silently hid every ""-stored non-reply (~1/3 of items).
      inReplyTo: { $in: [null, ""] },
      visibility: "public",
    };
    ...
    await enrichAccountStats(statuses, pluginOpts, baseUrl, req.app.locals.mastodonCollections);
...
router.get("/api/v1/timelines/tag/:hashtag", async (req, res, next) => {
  try {
    ...
    const baseFilter = {
      ...
      // Case-insensitive tag match (Mastodon tags are case-insensitive; stored
      // categories are mixed-case e.g. "AI"/"ATproto"). Anchored regex matches
      // both string and array `category` fields. Keep stored case as-is.
      category: new RegExp(
        `^${hashtag.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`,
        "i",
      ),
