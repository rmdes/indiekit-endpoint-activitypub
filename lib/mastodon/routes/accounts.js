...
router.get("/api/v1/accounts/:id/statuses", async (req, res, next) => {
  try {
    ...
    if (req.query.exclude_replies === "true") {
      // missing, null, or legacy "" all mean "not a reply" (see timelines.js)
      baseFilter.inReplyTo = { $in: [null, ""] };
    }
...
router.get("/api/v1/accounts/:id/featured_tags", async (req, res, next) => {
  try {
    ...
    const tags = await collections.ap_featured_tags.find({}).toArray();
    res.json(tags.map((t) => ({
...
async function serializeMemberUrls(memberUrls, collections, baseUrl) {
  ...
  const known = new Map();
  for (const col of [collections.ap_followers, collections.ap_following]) {
    if (!col) continue;
    const docs = await col.find({ actorUrl: { $in: memberUrls } }).toArray();
    for (const d of docs) {
      if (!known.has(d.actorUrl)) known.set(d.actorUrl, d);
    }
  }
