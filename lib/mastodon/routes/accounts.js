...
import { getEngagementCounts, loadInteractionState } from "../../core/interactions.js";
...
router.get("/api/v1/accounts/:id/statuses", async (req, res, next) => {
  try {
    ...
    if (req.mastodonToken) {
      ({ favouritedIds, rebloggedIds, bookmarkedIds, counts } =
        await loadInteractionState(collections, items));
    } else {
      counts = await getEngagementCounts(collections, items);
    }
