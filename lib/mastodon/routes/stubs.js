...
import { serializeFeaturedTags } from "../entities/featured-tag.js";
import { list } from "../../core/collections-io.js";
...
router.get("/api/v1/featured_tags", tokenRequired, scopeRequired("read", "read:accounts"), async (req, res, next) => {
  try {
    const collections = req.app.locals.mastodonCollections;
    if (!collections.ap_featured_tags) return res.json([]);

    const pluginOptions = req.app.locals.mastodonPluginOptions || {};
    const tags = await list(collections.ap_featured_tags, { filter: {} });
    res.json(serializeFeaturedTags(tags, pluginOptions.publicationUrl));
  } catch (error) {
    next(error);
  }
});
...
router.get("/api/v1/followed_tags", async (req, res, next) => {
  try {
    ...
    // Mastodon's followed_tags means local follows; a tags.pub-only follow is not one.
    const tags = (await getFollowedTags(collections)).filter((doc) => doc.followedAt);
