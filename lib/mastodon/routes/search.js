...
import { addTimelineItem, getDistinctTags, searchPosts } from "../../core/timeline.js";
...
router.get("/api/v2/search", tokenRequired, scopeRequired("read", "read:search"), async (req, res, next) => {
  try {
    ...
    if (!type || type === "accounts") {
      ...
      const accountDocs = await searchRelationships(collections, query, { limit });
    ...
    if (!type || type === "statuses") {
      const items = (await searchPosts(collections, query, {
        limit: limit + offset,
      })).slice(offset);
    ...
    if (!type || type === "hashtags") {
      ...
      // Distinct category values, filtered here — core has no opinion about
      // how a hashtag query is spelled.
      const allCategories = await getDistinctTags(collections);
