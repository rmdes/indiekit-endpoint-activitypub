...
router.post("/api/v1/apps", async (req, res, next) => {
  try {
    ...
    await createApp(collections, doc);
...
router.get("/api/v1/apps/verify_credentials", async (req, res, next) => {
  try {
    ...
    const collections = req.app.locals.mastodonCollections;
    const app = await findApp(collections, {
...
router.get("/oauth/authorize", async (req, res, next) => {
  try {
    ...
    const collections = req.app.locals.mastodonCollections;
    const app = await findApp(collections, { clientId: client_id });
    ...
    // Require an authenticated owner session to see the consent page.
    // SECURITY: force_login must FORCE re-authentication (Mastodon semantics),
    // it must NEVER bypass the gate. The old `&& !force_login` let an attacker
    // pass ?force_login=1 to render the authorize page with no session, obtain a
    // CSRF token, POST approve, and exchange the code for a token that can act
    // as the owner over the fediverse (follow/block/like run via Fedify with the
    // owner's key). force_login is now handled by clearing the session below.
    const session = req.session;
    if (force_login && session?.access_token) {
    ...
    // Render simple authorization page. Deny framing — this is a consent page;
    // a logged-in owner must not be clickjacked into approving a malicious app.
...
router.post("/oauth/authorize", async (req, res, next) => {
  try {
    ...
    // SECURITY: require an authenticated owner session before minting a code.
    // The GET consent gate can be bypassed by POSTing directly, so re-check
    // here — otherwise an anonymous request mints an auth code (indieauthToken
    // null) exchangeable for a token that can act as the owner over the
    // fediverse. Do NOT rely on the GET gate alone.
    if (!req.session?.access_token) {
    ...
    // Narrow requested scopes to what the app actually registered — a client
    // must not be granted scopes beyond its registration (RFC 6749 §3.3).
    const appScopes = app.scopes || ["read"];
    const grantedScopes = (scope ? scope.split(/\s+/) : appScopes)
      .filter((s) => appScopes.includes(s));

    await createToken(collections, {
      code,
      clientId: client_id,
      scopes: grantedScopes.length ? grantedScopes : appScopes,
  ...
  try {
    ...
    if (grant_type === "client_credentials") {
      ...
      await createToken(collections, {
        ...
        // App-level token: MUST NOT read the single owner's private data.
        // tokenRequired rejects appOnly tokens; only truly-public endpoints
        // (which don't use tokenRequired) are reachable with it.
        appOnly: true,
      });
    ...
    if (grant_type === "refresh_token") {
      ...
      // NOTE: refresh-token expiry is intentionally NOT enforced. Access tokens
      // don't expire (Mastodon parity), and clients that DO refresh must always
      // succeed unless the token was explicitly revoked — enforcing the 90-day
      // refreshExpiresAt rejected long-lived sessions and forced re-auth on a
      // single-user server for no real benefit. Only `revokedAt` gates refresh.
      const existing = await findToken(collections, {
    ...
    const grant = await consumeAuthorizationCode(collections, code);
    ...
    // Bind the code to the client it was issued to. Without this, a code
    // leaked to another party could be redeemed under a different client_id
    // (RFC 6749 §4.1.3). PKCE (above) already binds the code to the verifier
    // for public clients; this covers the client_id dimension too.
    const { clientId: presentedClientId } = extractClientCredentials(req);
    if (grant.clientId && presentedClientId && grant.clientId !== presentedClientId) {
      return res.status(400).json({
        error: "invalid_grant",
    ...
    await updateToken(
      ...
      // CRITICAL: this doc started life as the authorization CODE, which
      // carries a 10-minute `expiresAt`. Reusing it in-place as the access
      // token doc leaks that expiry onto the (permanent) access token, so
      // resolveToken() 401s every request ~10 min after login. Access tokens
      // never expire (Mastodon parity) — clear the inherited code expiry.
      // The refresh_token grant unsets it too; the code exchange must as well.
      ["expiresAt"],
    );
