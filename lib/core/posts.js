...
/**
 * What an AS2 request for one of our URLs resolves to: a live post, the
 * Tombstone of a deleted one (FEP-4f05), or neither.
 *
 * Tolerates a trailing slash on BOTH lookups — posts are stored without one,
 * but AS2 dereferences (and nginx passthrough) can add it.
 *
 * @param {object} collections - needs `posts`, `ap_tombstones`
 * @param {string} url
 * @returns {Promise<{post: object|null, tombstone: object|null}>}
 */
export async function resolvePostUrl(collections, url) {
  const bare = (url || "").replace(/\/$/, "");
  const variants = [bare, `${bare}/`];

  const post = collections.posts
    ? await collections.posts.findOne({ "properties.url": { $in: variants } })
    : null;

  if (post && !post.properties?.deleted) {
    return { post, tombstone: null };
  }

  const tombstone = collections.ap_tombstones
    ? await collections.ap_tombstones.findOne({ url: { $in: variants } })
    : null;

  return { post: null, tombstone };
}
