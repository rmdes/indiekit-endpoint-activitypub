 ...
 *
 * The reader's inbox/compose operations live in lib/storage/messages.js and are
 * re-exported here so adapters depend on core only (core → storage).
 *
...
/**
 * One message by its AP object id.
 *
 * @param {object} collections
 * @param {string} uid
 * @returns {Promise<object|null>}
 */
export async function getMessage(collections, uid) {
  if (!collections.ap_messages || !uid) return null;
  return collections.ap_messages.findOne({ uid });
}
