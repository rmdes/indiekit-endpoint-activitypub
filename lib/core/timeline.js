...
// Ingest (dedup, receivedAt/readAt stamping) stays in storage; adapters reach it
// through core.
export { addTimelineItem } from "../storage/timeline.js";
...
export function buildTimelineFilter(options = {}) {
  ...
  // Mutually exclusive with `type` by construction — a caller excluding boosts
  // is not also naming one specific type.
  if (options.excludeBoosts) {
    filter.type = { $ne: "boost" };
  }
...
export async function countNewer(collections, after, options = {}) {
  const base = buildTimelineFilter(options);
  const { filter, cursored } = await buildPage(collections.ap_timeline, base, {
    after,
  });
...
/**
 * Set fields on one item, addressed by AP URI or display URL — used to persist
 * data re-fetched from the origin (missing media, resolved quote).
 *
 * @param {object} collections
 * @param {string} uid - uid or url
 * @param {object} fields - dotted paths allowed, applied with $set
 * @returns {Promise<void>}
 */
export async function updateItemFields(collections, uid, fields) {
  if (!collections.ap_timeline || !uid || !fields || Object.keys(fields).length === 0) {
    return;
  }
  await collections.ap_timeline.updateOne({ $or: [{ uid }, { url: uid }] }, { $set: fields });
}
