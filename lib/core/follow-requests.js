...
/**
 * Pending follow requests, newest first.
 *
 * @param {object} collections
 * @param {object} [page]
 * @param {number} [page.skip=0]
 * @param {number} [page.limit=0] - 0 = all
 * @returns {Promise<object[]>}
 */
export async function getPendingFollows(collections, { skip = 0, limit = 0 } = {}) {
  if (!collections.ap_pending_follows) return [];

  let cursor = collections.ap_pending_follows.find({}).sort({ requestedAt: -1, _id: -1 });
 ...
 */
export async function countPendingFollows(collections) {
  if (!collections.ap_pending_follows) return 0;
  return collections.ap_pending_follows.countDocuments({});
}
