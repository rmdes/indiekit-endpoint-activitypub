...
/**
 * Resolve a Bearer access token to the token document it authorises, or null.
 *
 * Null for: unknown or revoked tokens, tokens past an `expiresAt`, and
 * app-only (client_credentials) tokens — those must not reach owner-private
 * data, so endpoints guarded by tokenRequired treat them as unauthenticated.
 *
 * @param {object} collections
 * @param {string} accessToken
 * @returns {Promise<object|null>}
 */
export async function resolveAccessToken(collections, accessToken) {
  if (!accessToken || !collections.ap_oauth_tokens) return null;
  ...
  // new Date() accepts a stored Date or an ISO string alike.
  if (token.expiresAt && new Date(token.expiresAt) < new Date()) return null;
  if (token.appOnly) return null;
