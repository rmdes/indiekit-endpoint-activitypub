...
/**
 * Pinned instance tabs, in display order — the hashtag explorer queries these.
 *
 * @param {object} collections
 * @param {{limit?: number}} [options]
 * @returns {Promise<{tabs: object[], total: number}>} `total` counts all instance tabs
 */
export async function getInstanceTabs(collections, { limit = 0 } = {}) {
  const tabs = collections.ap_explore_tabs;
  if (!tabs) return { tabs: [], total: 0 };

  let cursor = tabs.find({ type: "instance" }).sort({ order: 1 });
  if (limit) cursor = cursor.limit(limit);

  const [list, total] = await Promise.all([
    cursor.toArray(),
    tabs.countDocuments({ type: "instance" }),
  ]);
  return { tabs: list, total };
}
