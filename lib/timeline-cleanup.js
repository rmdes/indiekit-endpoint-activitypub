...
export async function cleanupTimeline(collections, retentionLimit) {
  ...
  // Prunable = remote rows, excluding replies to our own posts: those are the
  // conversation on our posts (stored as isContext since beta.6), not feed
  // volume, and losing them empties our threads.
  // ponytail: unbounded by timelineRetention — fine at personal-site reply
  // rates; if a pile-on ever bloats ap_timeline, add an age cap on these rows.
  const escapedOwner = ownerUrl?.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const remoteFilter = ownerUrl
    ? {
        "author.url": { $ne: ownerUrl },
        inReplyTo: { $not: new RegExp(`^${escapedOwner}`) },
      }
    : {};
  ...
  // Keep the newest by ARRIVAL (DD-1), matching what every feed shows. Sorting
  // on `published` deleted rows that had just arrived with an old date — the
  // top of the timeline — on every startup.
  const pipeline = [
    { $match: remoteFilter },
