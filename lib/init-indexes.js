...
export function createIndexes(collections, options) {
  ...
  try {
    ...
    // TTL index for MongoKvStore entries that carry a ttl (Fedify idempotency /
    // document-loader cache). Sparse so ttl-less KV entries (keypairs, job state)
    // never expire. expireAfterSeconds:0 = expire at the time in `expireAt`.
    collections.ap_kv?.createIndex(
      { expireAt: 1 },
