...
export function migrateGetController(mountPath, pluginOptions) {
  return async (request, response, next) => {
    try {
      ...
      const profile =
        (await getProfile({ ap_profile: application?.collections?.get("ap_profile") })) || {};
...
export function migratePostController(mountPath, pluginOptions) {
  return async (request, response, next) => {
    try {
      const { application } = request.app.locals;
      const collections = { ap_profile: application?.collections?.get("ap_profile") };
      let result = null;

      const aliasUrl = request.body.aliasUrl?.trim();
      if (aliasUrl && collections.ap_profile) {
        await updateProfile(collections, { alsoKnownAs: [aliasUrl] });
        result = {
          type: "success",
