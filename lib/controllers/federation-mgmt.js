...
import { countCollections, getRecentActivities as getActivityLog } from "../core/profile.js";
import { countOwnPosts, getOwnPostByUrl, getOwnPostsPage } from "../core/posts.js";
import {
  getAllMutedRows,
  getBlockedAccountRows,
    ...
    try {
      ...
      const [collectionStats, postsResult, recentActivities, blockedServers, blockedAccounts, mutedAccounts] =
        await Promise.all([
          ...
          getBlockedServerRows(pluginCollections),
          getBlockedAccountRows(pluginCollections),
          getAllMutedRows(pluginCollections),
        ]);
...
export function rebroadcastController(mountPath, plugin) {
  return async (request, response, next) => {
    try {
      ...
      const post = await getOwnPostByUrl({ posts }, url);
      if (!post) {
        return response
          .status(404)
...
async function getCollectionStats(collections, { redisUrl = "" } = {}) {
  ...
  const byName = Object.fromEntries(AP_COLLECTIONS.map((n) => [n, collections.get(n)]));
  const stats = await countCollections(byName, AP_COLLECTIONS);

  // When Redis handles KV, count fedify::* keys from Redis instead
  if (!redisUrl) return stats;
  const kvCount = await countRedisKvKeys(redisUrl);
  return stats.map((s) =>
    s.name === "ap_kv" ? { name: "ap_kv (redis)", count: kvCount } : s,
  );
}
...
async function getPaginatedPosts(collections, pageParam) {
  const scoped = { posts: collections?.get("posts") };
  if (!scoped.posts) return { posts: [], cursor: null };

  const page = Math.max(1, Number.parseInt(pageParam, 10) || 1);
  const totalCount = await countOwnPosts(scoped);
  const totalPages = Math.ceil(totalCount / PAGE_SIZE);

  const rawPosts = await getOwnPostsPage(scoped, {
...
async function getRecentActivities(collections) {
  return getActivityLog({ ap_activities: collections?.get("ap_activities") }, { limit: 5 });
}
