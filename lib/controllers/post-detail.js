...
import { Article, Note, Person, Service, Application } from "@fedify/vocab";
...
import { getItem, updateItemFields } from "../core/timeline.js";
import { getEngagementCounts, getInteractionRows } from "../core/interactions.js";

// Load replies (best-effort) — local descendants merged with the remote replies collection
export async function loadReplies(object, ctx, documentLoader, collections, maxReplies = 20, item = null) {
  const postUrl = object?.id?.href || object?.url?.href;

  // Start with replies already in our timeline (from organic inbox delivery or
  // reply chain fetching) — core/threads#getDescendants, the same direct +
  // nested set the Mastodon /context endpoint returns (AP-D4). Fast and free.
  // Needs only the stored item, so it runs even when no remote object was
  // fetched (a stored post with media skips the lookup).
  const seenUrls = new Set();
  const target = { uid: item?.uid || postUrl, url: item?.url || postUrl };
  const replies = collections.ap_timeline && (target.uid || target.url)
    ? await getDescendants(collections, target, { limit: maxReplies })
    : [];
      ...
      if (repliesCollection) {
        ...
        for (const replyItem of items.slice(0, maxReplies - replies.length)) {
          try {
            ...
            let reply = collections.ap_timeline
              ? await getItem(collections, replyUrl)
              : null;
  ...
  return async (request, response, next) => {
    try {
      ...
      const collections = {
        ...
        ap_interactions: application?.collections?.get("ap_interactions"),
      ...
      let timelineItem = collections.ap_timeline
        ? await getItem(collections, objectUrl)
        : null;
          ...
          if (object instanceof Note || object instanceof Article) {
            try {
              ...
              if (storedHasNoMedia) {
                ...
                if (hasMedia) {
                  await updateItemFields(collections, objectUrl, {
                    photo: freshItem.photo,
                    video: freshItem.video,
                    audio: freshItem.audio,
                  }).catch(() => {});
                }
              }
      ...
      if (timelineItem) {
        ...
        const interactions = await getInteractionRows(collections, [uid, displayUrl]);
      ...
      try {
        ...
        const threadPromise = Promise.all([
          // AP-D4: ancestors come from core/threads.js, shared with the
          // Mastodon lane. The reader passes a ctx, so it ALSO fetches missing
          // ancestors from their origin — the capability the Mastodon lane
          // lacks. What is no longer different is the DEPTH: core walks up to
          // MAX_ANCESTORS (40) for both, where this used to stop at 5 and show
          // fewer ancestors than the phone on a long thread.
          timelineItem.inReplyTo
            ? getAncestors(collections, timelineItem.inReplyTo, { ctx, documentLoader })
            : Promise.resolve([]),
          // Local descendants come from core (shared with Mastodon /context);
          // the reader additionally walks the object's remote replies collection.
          loadReplies(object, ctx, documentLoader, collections, 20, timelineItem),
        ]);

        const timeout = new Promise((resolve) =>
          setTimeout(() => resolve([[], []]), REMOTE_FETCH_TIMEOUT_MS),
        );
      ...
      if (timelineItem.quoteUrl && !timelineItem.quote) {
        try {
          ...
          if (quoteObject) {
            ...
            const persisted = { quote: timelineItem.quote };
            if (timelineItem.content?.html) {
              persisted["content.html"] = timelineItem.content.html;
            }
            updateItemFields(collections, objectUrl, persisted).catch(() => {});
          }
        } catch (error) {
      ...
      // Engagement counts — same rule as the reader cards and the Mastodon API
      const shown = [timelineItem, ...parentPosts, ...replyPosts];
      const mine = interactionMap[timelineItem.uid] || {};
      const counts = await getEngagementCounts(collections, shown, {
        favouritedIds: new Set(mine.like ? [timelineItem.uid] : []),
        rebloggedIds: new Set(mine.boost ? [timelineItem.uid] : []),
      });
      const withCounts = (item) => {
        const engagement = counts.get(item.uid || item.url);
        return engagement ? { ...item, counts: { ...item.counts, ...engagement } } : item;
      };
      ...
      response.render("activitypub-post-detail", {
        title: response.locals.__("activitypub.reader.post.title"),
        readerParent: { href: `${mountPath}/admin/reader`, text: response.locals.__("activitypub.reader.title") },
        item: withCounts(timelineItem),
        interactionMap,
        csrfToken,
        mountPath,
        parentPosts: parentPosts.map(withCounts),
        replyPosts: replyPosts.map(withCounts),
      });
    } catch (error) {
      next(error);
    }
