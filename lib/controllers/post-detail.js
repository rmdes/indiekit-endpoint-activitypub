...
import { getAncestors, REMOTE_FETCH_TIMEOUT_MS } from "../core/threads.js";

// Load local replies from ap_timeline (items where inReplyTo matches this post)
// Exported alongside loadParentChain — see the note there.
export async function loadLocalReplies(timelineCol, postUrl, postUid, maxReplies = 20) {
      ...
      try {
        ...
        const threadPromise = Promise.all([
          ...
          timelineItem.inReplyTo
            ? getAncestors(
                { ap_timeline: timelineCol },
                timelineItem.inReplyTo,
                { ctx, documentLoader },
              )
            : Promise.resolve([]),
          // Replies still come from the local+remote reader path, which walks
          // the object's own replies collection — richer than core's local
          // descendants query. Folding the two is Stage 4 work.
          object
