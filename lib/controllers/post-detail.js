...
import { getInteractionRows } from "../core/interactions.js";

// Load replies (best-effort) — local descendants merged with the remote replies collection
export async function loadReplies(object, ctx, documentLoader, collections, maxReplies = 20) {
  ...
  const replies = collections.ap_timeline && postUrl
    ? await getDescendants(collections, { uid: postUrl, url: postUrl }, { limit: maxReplies })
    : [];
    ...
    try {
      ...
      try {
        ...
        const threadPromise = Promise.all([
          ...
          object
            ? loadReplies(object, ctx, documentLoader, collections)
            : Promise.resolve([]),
        ]);
