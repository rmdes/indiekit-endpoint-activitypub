...
//
// Exported for the Stage 0 parity suite, which compares this against the
// Mastodon lane's GET /api/v1/statuses/:id/context (AP-D4: this defaults to
// maxDepth=5, that hard-caps at 40). Stage 3 moves it to core/threads.js —
// exporting is the first half of that extraction, not a test-only hook.
export async function loadParentChain(ctx, documentLoader, timelineCol, parentUrl, maxDepth = 5) {
  const parents = [];
