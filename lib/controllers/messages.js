...
import {
  ...
  countUnread as getUnreadMessageCount,
  markConversationRead as markMessagesRead,
  markAllMessagesRead,
  clearAllMessages,
  deleteMessage,
  addMessage,
...
/**
 * Build the Create(Note) for a direct message: addressed to the recipient only,
 * no Public collection.
 *
 * @param {object} options
 * @param {URL} options.actorUri - our actor
 * @param {{id: URL, preferredUsername?: string}} options.recipient
 * @param {string} options.noteId
 * @param {string} options.htmlContent - already sanitised
 * @param {string} [options.replyTo]
 * @param {import("@js-temporal/polyfill").Temporal.Instant} options.now
 * @returns {{note: Note, create: Create}}
 */
export function buildDirectMessage({ actorUri, recipient, noteId, htmlContent, replyTo, now }) {
  const note = new Note({
    id: new URL(noteId),
    // `attribution`, not `attributedTo` — the JSON-LD name is silently dropped
    // by Fedify's constructor (gotcha #41).
  ...
  return async (request, response, next) => {
    try {
      ...
      const [result, partners, unreadCount, myProfile] = await Promise.all([
        ...
        getProfile({ ap_profile: application?.collections?.get("ap_profile") }),
      ]);
...
export function submitMessageController(mountPath, plugin) {
  return async (request, response, next) => {
    try {
      ...
      const { create } = buildDirectMessage({
        actorUri: ctx.getActorUri(handle),
        recipient,
        noteId,
        htmlContent,
        replyTo,
