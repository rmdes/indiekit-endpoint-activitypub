...
import { deleteByAuthor } from "../core/timeline.js";
...
export function muteController(mountPath, plugin) {
  return async (request, response, next) => {
    try {
      ...
      await mute(collections, { url: url || undefined, keyword: keyword || undefined });
...
export function unmuteController(mountPath, plugin) {
  return async (request, response, next) => {
    try {
      ...
      const collections = getModerationCollections(request);
      await unmute(collections, { url: url || undefined, keyword: keyword || undefined });
...
export function blockController(mountPath, plugin) {
  return async (request, response, next) => {
    try {
      ...
      // Store the block
      await blockAccount(collections, url);

      // Remove timeline items from this actor — blocking removes existing
      // posts, which is what separates it from muting.
      await deleteByAuthor(collections, url);

      // Send Block activity via federation
      if (plugin._federation) {
        try {
          const { Block } = await import("@fedify/vocab");
...
export function unblockController(mountPath, plugin) {
  return async (request, response, next) => {
    try {
      ...
      const collections = getModerationCollections(request);
      await unblockAccount(collections, url);

      // Send Undo(Block) via federation
      if (plugin._federation) {
        try {
          const { Block, Undo } = await import("@fedify/vocab");
          const handle = plugin.options.actor.handle;
          const ctx = plugin._federation.createContext(
            new URL(plugin._publicationUrl),
            { handle, publicationUrl: plugin._publicationUrl },
...
export function blockServerController(mountPath) {
  return async (request, response, next) => {
    try {
      ...
      const collections = getModerationCollections(request);
      await blockServer(collections, hostname, reason);
...
export function unblockServerController(mountPath) {
  return async (request, response, next) => {
    try {
      ...
      const collections = getModerationCollections(request);
      await unblockServer(collections, hostname);
  ...
  return async (request, response, next) => {
    try {
      ...
      const [muted, blocked, blockedServers, filterMode] = await Promise.all([
        ...
        getBlockedAccountRows(collections),
