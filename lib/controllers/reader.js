...
import {
  countUnread as countUnreadItems,
  getTimeline as getTimelineItems,
} from "../core/timeline.js";
import {
  getNotifications,
  countUnread as getUnreadNotificationCount,
  countByType as getNotificationCountsByType,
  markRead as markAllNotificationsRead,
  deleteNotification,
...
export function readerController(mountPath) {
  return async (request, response, next) => {
    try {
      ...
      // Build query options
      const options = { before, after, limit, unreadOnly: unread };
      ...
      const { items, interactionMap } = await postProcessItems(tabFiltered, {
        ...
        // Engagement counts (same rule as the Mastodon API)
        collections: {
          ap_profile: application?.collections?.get("ap_profile"),
      ...
      let followedTags = [];
      try {
        followedTags = (await getFollowedTags(collections)).map((t) => t.tag);
      } catch {
...
export function notificationsController(mountPath) {
  ...
  return async (request, response, next) => {
    try {
      ...
      if (tab !== "all") {
        // core takes a types ARRAY; the reader's tabs are single types.
        options.types = [tab];
      }
